(function() {
  ...
  this.app = angular.module('widget', ['ui.bootstrap']).value('$anchorScroll', angular.noop);
...
(function() {
  ...
  this.PetitionController = function($scope, petition, PetitionServices, $http, Util, $rootScope, $interpolate) {
    ...
    $scope.getSignaturePanelHeight = function() {
      return $("#sign-panel").height();
    };
    $scope.getDeliverPanelHeight = function() {
      return $("#deliver-panel").height();
    };
    $scope.$watch($scope.getWidth, function(newValue, oldValue) {
      return console.log("browser width changed");
    });
    $scope.$watch($scope.getSignaturePanelHeight, function(newValue, oldValue) {
      ...
      return $("#deliver-panel").css("top", $("#sign-panel").height());
    });
    $scope.$watch($scope.getDeliverPanelHeight, function(newValue, oldValue) {
      console.log("deliver panel height changed");
      return $("#more-actions-panel").css("top", $("#deliver-panel").position().top + $("#deliver-panel").height());
    });
  ...
  PetitionController.resolve = {
    petition: [
      'PetitionServices', '$q', '$rootScope', function(PetitionServices, $q, $rootScope) {
        var deferred, petition_id, script_tag;
        deferred = $q.defer();
        script_tag = document.getElementById('snaptivist-widget');
        petition_id = script_tag.getAttribute("petition-id");
        PetitionServices.get_petition(petition_id).then(function(response) {
...
(function() {
  ...
  this.SignatureController = function($scope, PetitionServices, $http, Util, $rootScope) {
    ...
    return $scope.sign_with_facebook = function(auth) {
    ...
    '$http', '$q', '$rootScope', function($http, $q, $rootScope) {
      return {
        get_comments: function(petition_id, offset) {
          ...
          deferred = $q.defer();
          $http.get('http://www.snaptivist.net/api/petitions/' + petition_id + '/signatures?offset=' + offset).success(function(response) {
            if (response.statusCode === 200) {
              return deferred.resolve(response);
            } else {
    ...
    '$http', '$q', '$rootScope', function($http, $q, $rootScope) {
      return {
        get_petition: function(petition_id) {
          ...
          deferred = $q.defer();
          $http.jsonp('http://investigate-benghazi.snaptivist.net/api/petitions?callback=JSON_CALLBACK', {
            params: {
              id: petition_id
            }
          }).success(function(response) {
