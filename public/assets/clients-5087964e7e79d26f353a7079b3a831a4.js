...
(function() {
  ...
  this.app = angular.module('clients', ['contenteditable', 'angularFileUpload', 'ui.bootstrap']);
...
(function() {
  ...
  this.PetitionSetupController = function($scope, $route, $modal, $log, $rootScope, $location, fileReader, PetitionServices, layouts) {
    var lastRoute;
    window.scope = $scope;
    $scope.client_id = 1;
    $scope.is_admin = true;
    ...
    $scope.next_step_clicked = function(form) {
      ...
      if (form.$valid) {
        ...
        return $location.hash($scope.step);
      } else {
        errors = [];
        if ($scope.step === 1) {
    ...
    $scope.submit_petition = function() {
      var client_id;
      client_id = 1;
      return PetitionServices.create(client_id, $scope.petition, $scope.image_full, $scope.premium_image).success(function(response) {
        console.log("petitioin created");
        $scope.petition.result = response;
        $scope.step += 1;
        return $location.hash($scope.step);
      }).error(function() {
        return console.log("create give flow failed");
      });
    };
    ...
    return PetitionSetupController.$inject = ['$scope', '$route', '$modal', '$log', '$rootScope', '$location', 'fileReader', 'PetitionServices', 'layouts'];
  };
...
(function() {
  ...
  this.SidebarController = function($scope, $rootScope, Util) {
    window.scope = $scope;
    $scope.client = {};
    $scope.view_campaign = function(give_flow) {
      $rootScope.give_flow = give_flow;
      return Util.navigate('/dashboard');
    };
