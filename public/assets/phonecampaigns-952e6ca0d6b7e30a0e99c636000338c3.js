(function() {
  ...
  this.app = angular.module('phonecampaign', ['ui.bootstrap']).value('$anchorScroll', angular.noop);

  this.app.config([
    '$routeProvider', function($routeProvider) {
      var base_page_url;
      base_page_url = '/call';
      return $routeProvider.when('/call', {
        templateUrl: '/client_views/call',
...
(function() {
  ...
  this.CallController = function($scope, PhoneCampaignServices, $http, Util, $rootScope) {
    $scope.log_call_with_facebook = function() {
      return Util.navigate("/deliver");
    };
    return $scope.log_call_with_email_address = function() {
      return Util.navigate("/deliver");
    };
  };
...
(function() {
  ...
  this.DeliveryController = function($scope, PhoneCampaignServices, $http, $q, Util, $rootScope) {
    window.scope = $scope;
...
(function() {
  ...
  this.MoreActionController = function($scope, PhoneCampaignServices, $http, Util, $rootScope, more_actions) {
    $scope.more_actions = more_actions;
    $scope.loading.show_spinner = false;
    $scope.sign_another = function(phonecampaign) {
      $scope.loading.show_spinner = true;
      return PhoneCamapginServices.sign_another(phonecampaign.phonecampaign_id).success(function(response) {
        ...
        if (response.statusCode === 200) {
          ...
          if (scope.phonecampaign.image_square) {
            $.extend(true, fb_message_obj, {
              picture: $scope.phonecampaign.image_square
            });
          } else {
            $.extend(true, fb_message_obj, {
    ...
    $scope.read_more = function(phonecampaign) {
      return Util.navigate_absolute(("http://" + phonecampaign.subdomain + ".") + window.location.hostname.split('.').slice(2 - window.location.hostname.split('.').length - 1).join('.') + ":" + (window.location.port ? window.location.port : ""), null, false);
    };
  ...
  MoreActionController.resolve = {
    more_actions: [
      'PhoneCampaignServices', '$q', '$rootScope', function(PhoneCampaignServices, $q, $rootScope) {
        var deferred;
        deferred = $q.defer();
        PhoneCampaignServices.get_more_actions().then(function(actions) {
          console.log("got some other actions");
          return deferred.resolve(actions);
        });
...
(function() {
  ...
  this.PhoneCampaignsController = function($scope, PhoneCampaignServices, $http, Util, $rootScope) {
...
(function() {
  ...
  this.app.factory("PhoneCampaignServices", [
    '$http', '$q', '$rootScope', function($http, $q, $rootScope) {
      return {
