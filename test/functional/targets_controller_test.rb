...
class TargetsControllerTest < ActionController::TestCase
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post :create, target: { email_address: @target.email_address, name: @target.name, title: @target.title, twitter_handle: @target.twitter_handle }
    end
  ...
  test "should update target" do
    put :update, id: @target, target: { email_address: @target.email_address, name: @target.name, title: @target.title, twitter_handle: @target.twitter_handle }
    assert_redirected_to target_path(assigns(:target))
  end
